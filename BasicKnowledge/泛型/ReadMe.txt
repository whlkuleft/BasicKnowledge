Generic-泛型
泛型就是用一个 T 类型（类型参数），用来满足多种不同类型需求
泛型是2.0出来的 2.0之前只能用父类

泛型方法：一个方法完成多种类型实体的查询 public T Get<T>(T t)
泛型类：一个类，满足不同类型的需求 public class GenericClass<T>
泛型接口：一个接口，满足不同类型的需求 public interface GenericInterface<T>
泛型委托：一个委托，满足不同类型的需求 public delegate void Genericdelegate<T>(T t)
